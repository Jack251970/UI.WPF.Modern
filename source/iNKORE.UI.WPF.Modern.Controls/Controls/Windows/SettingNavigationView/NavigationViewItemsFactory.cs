...
public class NavigationViewItemsFactory : ElementFactory
{
    ...
    protected override UIElement GetElementCore(ElementFactoryGetArgs args)
    {
        ...
        NavigationViewItem nvi;
        {
            ...
            {
                if (m_navigationViewItemPool.Count > 0)
                {
                    var nvi = m_navigationViewItemPool.Last();
                    m_navigationViewItemPool.RemoveLast();
                    return nvi;
                }
        ...
        if (m_itemTemplateWrapper != null)
        {
            if (m_itemTemplateWrapper is ItemTemplateWrapper itemTemplateWrapper)
            {
                ...
                {
                    Element = newContent as UIElement
                };
    ...
    static void UnlinkElementFromParent(ElementFactoryRecycleArgs args)
    {
        ...
        if (args.Parent is Panel panel)
        {
            ...
            if (children.IndexOf(args.Element, out int childIndex))
            {
    ...
    List<NavigationViewItem> m_navigationViewItemPool = [];
}
